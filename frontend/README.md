...
/rolling-plan                    → 26-Week Rolling Block Planner
/execution                       → Field Execution Monitor
...
- **AI Block Planner** (`/planner`) — the primary workstation: a 7-lane Gantt-style timeline
  ...
  **Compare Plans** (current vs AI plan metrics). Includes section seasonal risk (SRS) and weather indicators.
- **26-Week Rolling Planner** (`/rolling-plan`) — multi-month tactical & strategic possession horizon:
  visualizes progressive task carry-forward, machine overhaul cycles, and cyclic ultrasonic flaw testing (USFD)
  with seasonal weather modulation across the 26-week calendar.
- **Field Execution Monitor** (`/execution`) — tracks active and scheduled block possessions in real time:
  provides an **ActiveExecutionCard** and **RecordOutcomeDialog** allowing field supervisors to report
  actual possession outcomes (`COMPLETED`, `PARTIAL`, `ABANDONED`), time variance (minutes), and completion notes,
  transmitting directly to `POST /api/blocks/{id}/execute` to close the human-in-the-loop lifecycle.
- **Maintenance Task Matrix** (`/tasks`) — dense, filterable, paginated table over the maintenance
  dataset with reactive in-memory search and multi-facet filtering (department / severity / status / priority / overdue / search).
...
- **Administration** (`/admin`) — operational health checks, data-source inspector, corridor topology viewer,
  and the live audit log.
...
- **Flow F**: Execution Monitor → Active Block → Record Outcome → Feedback Calibration

All six flows are implemented end-to-end with real state changes, toasts and audit-log entries.
...
- `settingsStore` — active operator role identity for audit log traceability, sidebar collapse, command-palette open state.
