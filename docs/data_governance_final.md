...
| `train_timetable` | `data/raw/traffic/train_timetable.csv` | **SYNTHETIC** | 50000 rows | `d2da85440c0bc640` | Active | Synthetic schedule: 500 trains $\times$ 7 days $\times$ 68 sections. Grounded in corridor traffic patterns. **Covers 49 / 68 sections**. |
| `railway_train_details_original` | `data/raw/timetable/ogd/railway_train_details_original.csv` | **REAL** | 186124 rows | `ca6b9a677212601e` | Reference | Full national IR timetable from data.gov.in (11,113 unique trains). Contains national coverage including New Delhi. **Hash differs from active timetable**. |
...
| `mdps_model` | `data/models/mdps_model.pkl` | **SYNTHETIC LABELS** | 651768 bytes | `6d213bf130f20e08` | Active | GradientBoostingRegressor trained on synthetic priority labels. $R^2=0.9756$, MAE$=2.2861$. |
| `mdps_scaler` | `data/models/mdps_scaler.pkl` | **DERIVED** | 807 bytes | N/A | Active | Feature scaler fitted on MDPS training dataset. |
| `ppo_rescheduler_v1` | `ml/reinforcement_learning/artifacts/ppo_rescheduler_v1.pt` | **SIMULATION TRAINED** | 80359 bytes | `1af9f66acb6988d6` | Active | PPO Actor-Critic MLP trained for ~199k timesteps on RailwayDisruptionEnv. 5 actions. 12-dim state. Mean reward: 16.35. |
...
- **Original National OGD File**: `data/raw/timetable/ogd/railway_train_details_original.csv`  
  **SHA256**: `ca6b9a677212601e303f8ba8ea3f7639133a9ffa7f6208649cbae3140de84235`
- **Active Runtime Timetable File**: `data/raw/traffic/train_timetable.csv`  
  **SHA256**: `d2da85440c0bc640221705a82fdd2a5922bcf9a7f85bcdec0106b97e7ab6226c`

**Finding**: The files have distinct hashes because `railway_train_details_original.csv` is the complete 186,124-row national timetable downloaded from data.gov.in (covering all Indian Railways zones), whereas `train_timetable.csv` is a 50,000-row synthetic/derived dataset filtered and generated specifically for the Chennai Egmore $\rightarrow$ Thoothukudi planning corridor.

### Section Coverage Audit Findings
- Out of 68 planning sections (`SEC_001` to `SEC_068`), `train_timetable.csv` contains operational schedules for **49 unique sections** (`SEC_001` through `SEC_049`).
- Sections `SEC_050` through `SEC_068` do not contain scheduled traffic in the synthetic timetable dataset.
- **Classification**: Section Coverage is **PARTIAL (49/68 sections, 72.1% corridor coverage)**.
