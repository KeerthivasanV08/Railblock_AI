...
| **CLM-05** | Runtime Timetable | 50,000 schedule rows | 50,000 rows, 500 trains, 49 sections | `data/raw/traffic/train_timetable.csv` | `train_timetable.csv` | SHA256: `d2da85440c0bc640` | **PARTIAL** | P2 | Expand coverage to 68 sections in future |
| **CLM-06** | Timetable Section Coverage | 68 / 68 sections | 49 / 68 sections (`SEC_001`–`SEC_049`) | `data/raw/traffic/train_timetable.csv` | `train_timetable.csv` | pandas unique check: 49 sections | **PARTIAL** | P2 | Document 72.1% section coverage |
| **CLM-07** | MDPS Algorithm | ML Priority Scoring | GradientBoostingRegressor ($R^2=0.9756$) | `data/models/mdps_model.pkl` | `mdps_engine.py:L142` | `type(model)` = GradientBoostingRegressor | **VERIFIED** | INFO | Update docs from RF to GradientBoosting |
...
| **CLM-15** | 26-Week Rolling Planner | Backend dynamic planner | 43 blocks scheduled across W1–W12 | `data/outputs/...` | `rolling_26week_block_plan.csv` | CSV inspection: W1–W12 populated | **PARTIAL** | P2 | W13–W26 unallocated (rolling horizon) |
...
| **CLM-20** | Authentication / RBAC | Enterprise RBAC roles | Prototype header-based authentication stub | `backend/.../auth_routes.py` | `auth_routes.py:L35` | Code inspection: `NOT_CONNECTED` stub | **PROTOTYPE STUB** | P2 | Clearly document prototype auth stub |
| **CLM-21** | Frontend TypeScript | Clean strict compilation | Nitro build succeeds; `tsc` optional warnings | `frontend/tsconfig.json` | `package.json` | `npm run build` succeeds in 1.71s | **PARTIAL** | P3 | `exactOptionalPropertyTypes` warnings |
| **CLM-22** | Unit Test Suite | Backend unit/integration | 131 / 131 tests passing cleanly | `backend/tests` | `pytest` | Pytest run: 131 passed in 72s | **VERIFIED** | INFO | None |
